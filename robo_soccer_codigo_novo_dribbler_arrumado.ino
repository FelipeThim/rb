...
#define DRIBBLER_ARM_US 1000
#define DRIBBLER_LENTO_US 1170
#define DRIBBLER_PARTIDA_US 1250
#define DRIBBLER_FORTE_US 1300
...
int velocidadeLateralChute = 160;
int velocidadeVoltaNorteRapida = 180;
...
int dribblerMicroAtual = DRIBBLER_ARM_US;
...
const int DISTANCIA_INICIAR_CHUTE_LATERAL_CM = 50;
const int DISTANCIA_FINAL_CHUTE_LATERAL_CM = 25;
const int ANGULO_LADO_DIREITO = -90;
const int ANGULO_LADO_ESQUERDO = 90;
const int LIMIAR_LINHA = 70; // alterar para sensor de linha na quadra
const bool LINHA_VALOR_MENOR_QUE_QUADRA = true;
const bool CHUTE_LATERAL_PELA_DIREITA = true;
const unsigned long TEMPO_RE_LINHA_MS = 450;
const unsigned long TEMPO_MAX_CHUTE_LATERAL_MS = 3500;
...
void andar_ate_gol() {
  while (true) {
    ...
    Serial.println(ultimaDistanciaFrenteCm);

    if (ultimaDistanciaFrenteCm > 0 && ultimaDistanciaFrenteCm <= DISTANCIA_INICIAR_CHUTE_LATERAL_CM) {
      stop_robot();
      delay(200);

      Serial.println("Gol a menos de 50 cm. Fazendo chute lateral...");
      chutar_lateral_ate_gol();
...
void chutar_lateral_ate_gol() {
  int anguloLado = CHUTE_LATERAL_PELA_DIREITA ? ANGULO_LADO_DIREITO : ANGULO_LADO_ESQUERDO;

  dribbler_forte();
  girar_ate_angulo(anguloLado, velocidadeGiro, 4000);
  stop_robot();
  delay(100);
  ...
  while (true) {
    ...
    monitor_serial();
    atualizar_ultrassonicos();
    float distanciaLateral = ler_distancia_lateral_chute();

    Serial.print("Distancia lateral chute: ");
    Serial.println(distanciaLateral);

    if (distanciaLateral > 0 && distanciaLateral <= DISTANCIA_FINAL_CHUTE_LATERAL_CM) {
      stop_robot();
      delay(100);
      break;
    }

    if (millis() - inicio > TEMPO_MAX_CHUTE_LATERAL_MS) {
      stop_robot();
      Serial.println("Tempo limite do chute lateral. Voltando para o norte.");
      break;
    }

    mover_lateral_chute(velocidadeLateralChute);
    delay(40);
  }

  girar_ate_angulo(0, velocidadeVoltaNorteRapida, 2500);
  stop_robot();
  delay(200);
}

float ler_distancia_lateral_chute() {
  if (CHUTE_LATERAL_PELA_DIREITA) {
    return ultimaDistanciaDireitaCm;
  }

  return ultimaDistanciaEsquerdaCm;
}

void mover_lateral_chute(int v) {
  if (CHUTE_LATERAL_PELA_DIREITA) {
    move_right(v);
  } else {
    move_left(v);
...
void dribbler_armar() {
  esc.writeMicroseconds(DRIBBLER_ARM_US);
  dribblerMicroAtual = DRIBBLER_ARM_US;
