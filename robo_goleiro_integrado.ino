...
const bool USAR_CALIBRACAO_LINHA = true;
const int TEMPO_CALIBRACAO_LINHA_MS = 6000;
const int VARIACAO_MINIMA_LINHA = 80;
...
void setup() {
  ...
  if (USAR_CALIBRACAO_LINHA) {
    calibrar_linha_goleiro();
  } else {
    inicializar_limiar_fixo_linha();
  }
...
void inicializar_limiar_fixo_linha() {
  for (int i = 0; i < 16; i++) {
    valorMin[i] = BRANCO_VALOR_MAIOR ? 0 : 1023;
    valorMax[i] = BRANCO_VALOR_MAIOR ? 1023 : 0;
    limiar[i] = LIMIAR_BRANCO;
    sensorCalibrado[i] = true;
    linha[i] = false;
  }
}

void calibrar_linha_goleiro() {
  Serial.println("=== CALIBRACAO DA LINHA DO GOLEIRO ===");
  Serial.println("Coloque o robo perto da linha e mova para alguns sensores passarem no branco.");
  ...
  while (millis() - inicio < TEMPO_CALIBRACAO_LINHA_MS) {
    lerLDR();
  ...
  Serial.println();
  Serial.println("=== RESULTADO CALIBRACAO LINHA ===");

  for (int i = 0; i < 16; i++) {
    int variacao = valorMax[i] - valorMin[i];
    limiar[i] = (valorMin[i] + valorMax[i]) / 2;
    sensorCalibrado[i] = variacao >= VARIACAO_MINIMA_LINHA;
  ...
  Serial.println("Calibracao da linha concluida!");
}
...
void modo_gol_enquanto_fim_curso() {
  ...
  Serial.println("Fim de curso solto. Voltando ao modo goleiro.");
  estadoRobo = "Goleiro na linha";
  stop_robot();
}
...
void calcularDirecaoLinha() {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (USAR_CALIBRACAO_LINHA && sensorCalibrado[i]) {
      bool brancoEhMax = valorMax[i] > valorMin[i];
      branco = brancoEhMax ? ldr[i] >= limiar[i] : ldr[i] <= limiar[i];
    } else if (BRANCO_VALOR_MAIOR) {
