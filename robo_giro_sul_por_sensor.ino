...
// ===== TEMPOS =====
const unsigned long TEMPO_ENCAIXAR_BOLA_MS = 500;
const unsigned long TEMPO_GIRO_COM_BOLA_MS = 800;
const unsigned long TEMPO_RE_LINHA_MS = 450;
const unsigned long TEMPO_PARADO_NO_SUL_MS = 500;
const unsigned long TEMPO_AVANCO_GOL_MS = 700;
const unsigned long INTERVALO_LOOP_MS = 10;
const unsigned long TIMEOUT_ULTRASSONICO_US = 12000;
const unsigned long TEMPO_DECIDIR_GIRO_SUL_MS = 500;
...
#define DRIBBLER_VELOCIDADE_PROCURA_US 1150
#define DRIBBLER_VELOCIDADE_COM_BOLA_US 1210

// ===== VELOCIDADES DA LOGICA COM BOLA =====
const int VELOCIDADE_GIRO_SUL_COM_BOLA = 80;
const int VELOCIDADE_GIRO_NORTE_LENTO = 170;
const int VELOCIDADE_GIRO_NORTE_RAPIDO = 210;
const int ANGULO_RESTANTE_PARA_GIRO_NORTE_RAPIDO = 75;
const int VELOCIDADE_AVANCO_GOL = 180;
...
void loop() {
  ...
  if (fimCursoAgora && !bolaCapturada && !ignorarFimCursoAteSoltar) {
    ...
    debug_println("Medindo ultrassonicos laterais parado no sul...");
    atualizar_ultrassonicos_laterais_media();
    ...
    bool ladoMaiorDireito = escolher_lado_maior_ultrassonico();
    bool voltarNorteParaEsquerda = !ladoMaiorDireito;

    debug_print("Lado maior escolhido: ");
    debug_println(ladoMaiorDireito ? "direita" : "esquerda");
    debug_print("Voltando para o norte pelo lado: ");
    debug_println(voltarNorteParaEsquerda ? "esquerda" : "direita");

    if (!olhar_para_norte_por_lado_com_bola(voltarNorteParaEsquerda)) {
      debug_println("Falha ao voltar para o norte pelo lado maior. Voltando para procurar.");
      dribbler_velocidade_procura();
      bolaCapturada = false;
      ignorarFimCursoAteSoltar = true;
      return;
...
void atualizar_ultrassonicos_laterais_media() {
  stop_robot();
  delay(100);
...
bool escolher_lado_maior_ultrassonico() {
  if (ultimaDistanciaDireitaCm <= 0 && ultimaDistanciaEsquerdaCm <= 0) {
    debug_println("Laterais sem leitura valida. Usando direita por padrao.");
    return true;
  }
  ...
  if (ultimaDistanciaEsquerdaCm <= 0) {
    return true;
  }

  return ultimaDistanciaDireitaCm > ultimaDistanciaEsquerdaCm;
}

void medir_ultrassonicos_laterais_por_tempo(unsigned long tempoMs) {
  unsigned long inicio = millis();
  float somaEsquerda = 0;
  float somaDireita = 0;
  ...
  while (millis() - inicio < tempoMs) {
    ...
    if (distanciaEsquerda > 0) {
      somaEsquerda += distanciaEsquerda;
      leiturasEsquerda++;
    }

    if (distanciaDireita > 0) {
      somaDireita += distanciaDireita;
      leiturasDireita++;
    }
  ...
  ultimaDistanciaEsquerdaCm = leiturasEsquerda > 0 ? somaEsquerda / leiturasEsquerda : 0;
  ultimaDistanciaDireitaCm = leiturasDireita > 0 ? somaDireita / leiturasDireita : 0;
}

bool escolher_giro_sul_para_esquerda_por_ultrassonico() {
  debug_println("Pensando por 1 segundo para escolher giro ate o sul...");
  medir_ultrassonicos_laterais_por_tempo(TEMPO_DECIDIR_GIRO_SUL_MS);

  debug_print("Media esquerda para giro sul: ");
  debug_println(ultimaDistanciaEsquerdaCm);
  debug_print("Media direita para giro sul: ");
  debug_println(ultimaDistanciaDireitaCm);

  if (ultimaDistanciaDireitaCm <= 0 && ultimaDistanciaEsquerdaCm <= 0) {
    debug_println("Sem decisao em 1 segundo. Giro sul para esquerda por padrao.");
    return true;
  }

  if (ultimaDistanciaDireitaCm <= 0) {
    debug_println("Direita sem leitura valida. Sem decisao completa. Giro sul para esquerda por padrao.");
    return true;
  }

  if (ultimaDistanciaEsquerdaCm <= 0) {
    debug_println("Esquerda sem leitura valida. Sem decisao completa. Giro sul para esquerda por padrao.");
    return true;
  }

  if (ultimaDistanciaDireitaCm == ultimaDistanciaEsquerdaCm) {
    debug_println("Leituras empatadas. Giro sul para esquerda por padrao.");
    return true;
  }

  bool direitaMaisPerto = ultimaDistanciaDireitaCm < ultimaDistanciaEsquerdaCm;
  debug_print("Lado mais perto para giro sul: ");
  debug_println(direitaMaisPerto ? "direita" : "esquerda");

  return !direitaMaisPerto;
}
...
bool olhar_para_sul_com_bola_lento() {
  bool girarParaEsquerda = escolher_giro_sul_para_esquerda_por_ultrassonico();

  debug_print("Sentido escolhido para o sul: ");
  debug_println(girarParaEsquerda ? "esquerda" : "direita");

  return girar_ate_angulo_com_bola_fixo_sentido(ANGULO_SUL, VELOCIDADE_GIRO_SUL_COM_BOLA, 7000, girarParaEsquerda);
}
...
bool olhar_para_norte_duas_etapas_com_bola() {
  ...
  while (true) {
    ...
    int velocidadeAtual = erroAbs > ANGULO_RESTANTE_PARA_GIRO_NORTE_RAPIDO ? VELOCIDADE_GIRO_NORTE_LENTO : VELOCIDADE_GIRO_NORTE_RAPIDO;
...
bool olhar_para_norte_por_lado_com_bola(bool paraEsquerda) {
  unsigned long inicio = millis();
  ...
  while (true) {
    ...
    if (erroAbs <= TOLERANCIA_ANGULO) {
      debug_println("Norte alcancado pelo lado maior.");
      stop_robot();
      return true;
    }
...
void avancar_para_gol_com_bola() {
  unsigned long inicio = millis();

  dribbler_velocidade_com_bola();

  while (millis() - inicio < TEMPO_AVANCO_GOL_MS) {
    atualizar_gyro();
    ...
    move_foward(VELOCIDADE_AVANCO_GOL);
    delay(10);
  }
