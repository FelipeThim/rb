...
void loop() {
  ...
  if (fim_curso_acionado() && !bolaCapturada && !ignorarFimCursoAteSoltar) {
    ...
    Serial.println("Bola capturada. Usando MPU para voltar ao angulo 0...");
    olhar_para_norte();
    ...
    Serial.println("Chute finalizado. Voltando para a logica de buscar/ficar atras da bola.");
    delay(300);
    return;
  }
...
void chutar_com_giro_lateral() {
  ...
  Serial.println("Chute: dribbler forte, virando devagar para a direita...");
  esc.writeMicroseconds(DRIBBLER_CHUTE_US);
  girar_ate_angulo(-ANGULO_CHUTE_DIREITA, VELOCIDADE_CHUTE_DIREITA, TEMPO_MAX_GIRO_CHUTE);

  Serial.println("Chute: voltando rapido para o norte...");
  girar_ate_angulo(0, VELOCIDADE_VOLTA_NORTE, TEMPO_MAX_GIRO_CHUTE);
