...
const int ANGULO_SUL = 0;      // Direcao salva quando liga o robo
const int ANGULO_NORTE = 180;  // Direcao oposta ao sul salvo
const int LIMIAR_LINHA = 300; // alterar para sensor de linha na quadra
...
const int VELOCIDADE_ALINHAMENTO_SUL_COM_BOLA = 160;
const int VELOCIDADE_ALINHAMENTO_NORTE_COM_BOLA = 230;
...
void loop() {
  ...
  if (fim_curso_acionado() && !bolaCapturada && !ignorarFimCursoAteSoltar) {
    ...
    debug_println("Bola capturada. Virando devagar para o sul salvo ao ligar...");
    ...
    stop_robot();
    debug_println("Sul alcancado. Pausando antes de virar rapido para o norte...");
    delay(TEMPO_PARADO_NO_SUL_MS);

    debug_println("Virando rapido para o norte para jogar a bola...");
    ...
    debug_println("Bola apontada para o norte. Liberando para procurar novamente.");
...
void monitor_serial() {
  ...
  debug_print("Erro para Norte / 180 graus: ");
  debug_println(erro_para_norte());

  debug_print("Erro para Sul / 0 graus: ");
...
bool olhar_para_sul_com_bola() {
  return girar_ate_angulo_com_bola(ANGULO_SUL, VELOCIDADE_ALINHAMENTO_SUL_COM_BOLA, 6000);
}

bool olhar_para_norte_com_bola() {
  return girar_ate_angulo_com_bola(ANGULO_NORTE, VELOCIDADE_ALINHAMENTO_NORTE_COM_BOLA, 6000);
}
...
bool girar_ate_angulo_com_bola(float alvo, int velocidade, unsigned long tempoMaximo) {
  ...
  while (true) {
    ...
    if (erro > 0) {
      ...
      debug_print(" erro=");
      debug_println(erro);
      left_rotation_alinhamento_com_bola(velocidade);
    } else {
      debug_print("MPU girando RIGHT | alvo=");
      debug_print(alvo);
      debug_print(" erro=");
      debug_println(erro);
      right_rotation_alinhamento_com_bola(velocidade);
    }
...
void left_rotation_alinhamento_com_bola(int velocidade) {
  left_rotation(velocidade);
}

void right_rotation_alinhamento_com_bola(int velocidade) {
  right_rotation(velocidade);
}
