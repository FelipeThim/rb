...
#define DRIBBLER_VELOCIDADE_PROCURA_US 1180
...
int velocidadeFrente = 250;
int velocidadeGiro = 110;
int velocidadeBusca = 140;
...
const int LIMIAR_LINHA = 200; // alterar para sensor de linha na quadra
const bool LINHA_VALOR_MENOR_QUE_QUADRA = false;
const bool SERIAL_ATIVO = false;
...
void loop() {
  ...
  if (fim_curso_acionado() && !bolaCapturada && !ignorarFimCursoAteSoltar) {
    ...
    stop_robot();
    dribbler_velocidade_procura();
    debug_println("Bola encostou no fim de curso. Esperando encaixar...");
    ...
    debug_println("Bola capturada. Virando para o sul...");
    if (!olhar_para_sul_com_bola()) {
      debug_println("Bola escapou ao alinhar. Voltando para procurar.");
      return;
    }
    ...
    debug_println("Virando de costas para preparar o chute...");
    if (!olhar_para_tras_do_gol_com_bola()) {
      debug_println("Bola escapou ao virar para tras. Voltando para procurar.");
      return;
    }
    ...
    debug_println("Chutando por giro rapido para esquerda...");
...
void monitor_serial() {
  ...
  debug_println(fim_curso_acionado() ? "ACIONADO" : "SOLTO");

  debug_print("Angulo MPU: ");
  debug_println(anguloRobo);

  debug_print("Erro para Sul: ");
...
bool correr_para_gol_com_bola() {
  while (fim_curso_acionado()) {
    ...
    if (ultimaDistanciaFrenteCm > 0 && ultimaDistanciaFrenteCm <= 25) {
      stop_robot();
      delay(200);
      return true;
    }
...
bool olhar_para_tras_do_gol_com_bola() {
  float anguloTrasDoGol = normalizar_angulo(ANGULO_SUL + 180);
  return girar_ate_angulo_com_bola(anguloTrasDoGol, velocidadeGiro, 6000);
}

void olhar_para_tras_do_gol() {
  float anguloTrasDoGol = normalizar_angulo(ANGULO_SUL + 180);
  girar_ate_angulo(anguloTrasDoGol, velocidadeGiro, 6000);
}
