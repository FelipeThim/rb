...
bool parede_muito_perto() {
  float distanciaFrente = ler_distancia_cm(echo_frente);
  delay(80);
  ...
  return (distanciaFrente > 0 && distanciaFrente <= DISTANCIA_PAREDE_PERTO_CM) ||
         (distanciaEsquerda > 0 && distanciaEsquerda <= DISTANCIA_PAREDE_PERTO_CM) ||
         (distanciaDireita > 0 && distanciaDireita <= DISTANCIA_PAREDE_PERTO_CM);
}
...
void andar_ate_distancia() {
  ...
  while (true) {
    ...
    float distancia = ler_distancia_cm(echo_frente);
...
void imprimir_ultrassonico() {
  float frente = ler_distancia_cm(echo_frente);
  delay(80);

  float esquerda = ler_distancia_cm(echo_esquerda);
  delay(80);

  float direita = ler_distancia_cm(echo_direita);
  delay(80);
