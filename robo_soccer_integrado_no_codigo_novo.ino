...
#define DRIBBLER_MAX_US 1200
#define DRIBBLER_CHUTE_US 2000
...
const int VELOCIDADE_CHUTE_DIREITA = 75;
const int VELOCIDADE_VOLTA_NORTE = 170;
const int ANGULO_CHUTE_DIREITA = 35;
const unsigned long TEMPO_MAX_GIRO_CHUTE = 1800;
...
void loop() {
  ...
  if (fim_curso_acionado() && !bolaCapturada && !ignorarFimCursoAteSoltar) {
    ...
    Serial.println("Finalizado. Voltando para buscar/ficar atras da bola.");
    return;
  }
...
void andar_ate_gol() {
  while (true) {
    ...
    if (distancia > 0 && distancia <= DISTANCIA_PARAR_CM) {
      ...
      Serial.println("Gol perto. Chutando com giro...");
      chutar_com_giro_lateral();
...
void olhar_para_norte() {
  girar_ate_angulo(0, velocidadeGiro, 6000, TOLERANCIA_NORTE);
}

void chutar_com_giro_lateral() {
  olhar_para_norte();
  delay(100);

  Serial.println("Chute: virando devagar para a direita...");
  esc.writeMicroseconds(DRIBBLER_CHUTE_US);
  girar_ate_angulo(-ANGULO_CHUTE_DIREITA, VELOCIDADE_CHUTE_DIREITA, TEMPO_MAX_GIRO_CHUTE, TOLERANCIA_CHUTE);

  Serial.println("Chute: voltando rapido para o norte...");
  girar_ate_angulo(0, VELOCIDADE_VOLTA_NORTE, TEMPO_MAX_GIRO_CHUTE, TOLERANCIA_CHUTE);
...
void girar_ate_angulo(float alvo, int velocidade, unsigned long tempoMaximo, int tolerancia) {
